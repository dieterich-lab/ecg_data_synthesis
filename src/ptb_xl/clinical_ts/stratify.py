...
def split_stratified(df_train, subset_ratios, filename="subset.txt", col_subset="subset", col_index=None,
                     col_label="label", col_group=None, label_multi_hot=False, random_seed=0):
    ...
    if os.path.exists(filename):  # load predefined splits
        ...
        df_train[col_subset] = stratified_subsets(df_train, col_label, subset_ratios, col_group,
                                                  label_multi_hot=label_multi_hot, random_seed=random_seed)
        # write file to disk
...
def stratified_subsets(df, col_label, subset_ratios, col_group=None, label_multi_hot=False, random_seed=0):
    ...
    if col_group is not None:  # col_group set- i.e. aggregate according to patients etc.
        ...
        group_ids = stratify(list(df_group[col_label]), classes, subset_ratios, list(df_group["samples_per_group"]),
        ...
        ids_subsets = stratify(list(df[col_label]) if multi_label else list(df[col_label].apply(lambda x: [x])),
                               classes, subset_ratios)
...
def stratify_batched(data, classes, ratios, samples_per_group, random_seed=0, verbose=True, batch_size=20000):
    """calls stratify in batches and collects results afterward (use only for really large datasets)"""
    num_data = len(data)
    ...
    for i in range(num_batches):
        ...
        end_idx.append(
            start_idx[-1] + batch_size + rest_final if i == num_batches - 1 else start_idx[-1] + batch_size + rest_div)

    res_final = None
    for s, e in tqdm(list(zip(start_idx, end_idx))):

        res = stratify(data[s:e], classes, ratios,
                       samples_per_group=samples_per_group[s:e] if samples_per_group is not None else None,
...
def stratify(data, classes, ratios, samples_per_group=None, random_seed=0, verbose=True):
    ...
    processed_data is a list of lists: a list of labels, for each sample (possibly containing duplicates not multi-hot encoded).
    ...
    # processed_data is now always a list of lists; len(processed_data) is the number of patients; processed_data[i] is the list of all labels for patient i (possibly multiple identical entries)
    ...
    # Organize processed_data per label: for each label l, per_label_data[l] contains the list of patients
    # in processed_data which have this label (potentially multiple identical entries)
    ...
    # For each sample in the processed_data set
    ...
    for _ in tqdm(list(range(len(classes)))):
        ...
        try:
            ...
            # "No label" could be a class in itself: it's up to you to formaxxxt your processed_data accordingly.
            break
        ...
        for current_id, current_count in tqdm(list(zip(unique_samples, unique_counts)), leave=False):
            ...
            # Find argmax clj i.e. subset in the greatest need of the current label
            ...
            # If there is more than one such subset, find the one in the greatest need of any label
            else:
    ...
    # stratified_data = [
    #    [processed_data[i] for i in strat] for strat in stratified_data_ids
    # ]
