...
# Cell
def resample_data(sigbufs, channel_labels, fs, target_fs, channels=12,
                  channel_stoi=None):  # ,skimage_transform=True,interpolation_order=3):
    ...
    if channel_stoi is not None:
        ...
        for i, cl in enumerate(channel_labels):
            if cl in channel_stoi.keys() and channel_stoi[cl] < channels:
                ...
                #    processed_data[:,channel_stoi[cl]]=transform.resize(sigbufs[:,i],(timesteps_new,),order=interpolation_order).astype(np.float32)
                # else:
                #    processed_data[:,channel_stoi[cl]]=zoom(sigbufs[:,i],timesteps_new/len(sigbufs),order=interpolation_order).astype(np.float32)
                data[:, channel_stoi[cl]] = resampy.resample(sigbufs[:, i], fs, target_fs).astype(np.float32)
    else:
        # if(skimage_transform):
        #    processed_data=transform.resize(sigbufs,(timesteps_new,channels),order=interpolation_order).astype(np.float32)
        # else:
        #    processed_data=zoom(sigbufs,(timesteps_new/len(sigbufs),1),order=interpolation_order).astype(np.float32)
    ...
    if target_folder is None:
        ...
        if ("train" in filename_in.parts):
            ...
            target_folder_train.mkdir(parents=True, exist_ok=True)
                ...
                "eval" in filename_in.parts or "dev_test" in filename_in.parts or "valid" in filename_in.parts or "valtest" in filename_in.parts):
...
def reformat_data_ptb(datafiles, target_fs=200, channels=12, channel_stoi=None, lbl_itos=None,
                      target_folder=None):
    if channel_stoi is None:
    ...
    for filename_in in tqdm(datafiles):
        ...
        diagnosis2 = [l.replace("Typ IIa", "Type IIa").replace("Hypertension", "hypertension").replace(
            " (intraventricular gradient 100-160mmHg)", "").replace("Fibrillation", "fibrillation").replace(
            "Recurrent ventricular tachycardias", "Recurrent ventricular tachycardia").replace("Thyriodectomy",
                                                                                               "Thyroidectomy").replace(
            "Dilated Cardiomyopathy secondary to coronary artery disease. Mitral regurgitation (grade 2)",
            "Dilated Cardiomyopathy") for l in diagnosis2]  # clean up
        # remove type from Hyperlipoproteinemia Type xx TODO
        diagnosis2 = [
            l.replace("Hyperlipoproteinemia Type IIa", "Hyperlipoproteinemia").replace("Hyperlipoproteinemia Type IV",
                                                                                       "Hyperlipoproteinemia").replace(
                "Hyperlipoproteinemia Type IIb", "Hyperlipoproteinemia") for l in diagnosis2]  # clean up
        ...
        row = {"processed_data": filename_out_relative, "dataset": "PTB-XL", "channels_available": channels_available,
               "patient": "PTB_" + patient, "ecg": "PTB_" + ecg, "age": age, "label_age": label_age, "sex": sex,
               "label_sex": label_sex, "ecg_date": metadata[2], "diagnosis": diagnosis1,
               "infarction_localization": metadata[5], "diagnosis2": diagnosis2, "diagnosis12": diagnosis12,
               "smoker": smoker, "label_smoker": label_smoker, "infarction_date": metadata[10],
    ...
    if (lbl_itos is not None):
        ...
        df_all["label"] = df_all.diagnosis.apply(lambda x: (lbl_stoi[x] if x in lbl_stoi.keys() else len(
            lbl_itos)))  # map everything that cannot be found in the dictionary to last entry
    else:
...
def _ptb_cleanup_hemodynamics(df_ptb):
    ...
    df_ptb["Peripheral blood Pressure (syst/diast)"] = df_ptb["Peripheral blood Pressure (syst/diast)"].apply(
        lambda x: x.replace("120 /70", "120/70"))
    ...
    _reformat_col(df_ptb, "Pulmonary artery pressure (at rest) (syst/diast)", "pulmonary_ap_at_rest_syst",
                  "pulmonary_ap_at_rest_diast")
    _reformat_col(df_ptb, "Pulmonary artery pressure (laod) (mean)", "pulmonary_load_mean")
    _reformat_col(df_ptb, "Pulmonary artery pressure (laod) (syst/diast)", "pulmonary_ap_load_syst",
    ...
    df_ptb["Left ventricular enddiastolic pressure"] = df_ptb["Left ventricular enddiastolic pressure"].apply(
        lambda x: x[-8:].strip())  # only post value
...
def prepare_data_ptb(data_folder, target_folder=None, channel_stoi=None, channels=12, target_fs=50,
                     strat_folds=10, cv_random_state=42, recreate_data=False, ptb_permanently_remove_diagnosis_na=True,
                     ptb_permanently_remove_multiple_ecgs=False, ptb_select_only_first_mi=True,
                     ptb_remove_unknown_loc_mi=True, lbl_itos_ptb_in=None,
                     lbl_itos_ptb_regression=None, lbl_itos_ptb_all=None):
    if channel_stoi is None:
        channel_stoi = channel_stoi_default
    if lbl_itos_ptb_all is None:
        lbl_itos_ptb_all = ["Diabetes mellitus"]
    if lbl_itos_ptb_in is None:
        lbl_itos_ptb_in = ["Healthy control", "Myocardial infarction"]
    if lbl_itos_ptb_regression is None:
    ...
    if recreate_data:
        ...
        # reformat processed_data
        df_ptb, lbl_itos_ptb = reformat_data_ptb(datafiles=datafiles_ptb, target_fs=target_fs, channels=channels,
                                                 lbl_itos=lbl_itos_ptb_in, target_folder=target_folder)

        df_ptb["infarction_localization_mapped"] = df_ptb["infarction_localization"].apply(
            lambda x: int_to_str[diagnosis_to_int_w_subdiagnosis[x]])
        df_ptb["diagnosis_extended"] = df_ptb.apply(
            lambda row: row["diagnosis"] + " " + row["infarction_localization_mapped"], axis=1)
        # TODO move all this stuff into reformat_data_ptb
        # set regression label(s)
        df_ptb["label_regression"] = df_ptb.apply(
            lambda row: row[lbl_itos_ptb_regression[0]] if len(lbl_itos_ptb_regression) == 1 else [row[regression_col]
                                                                                                   for regression_col in
                                                                                                   lbl_itos_ptb_regression],
            axis=1)
        ...
        df_ptb["label_multi_all"] = df_ptb.diagnosis12.apply(
            lambda x: extract_multilabel(x, lbl_itos_ptb_all) if len(lbl_itos_ptb_all) > 1 else int(
                lbl_itos_ptb_all[0] in x))
        ...
        if (ptb_permanently_remove_multiple_ecgs):
            ...
            df_ptb["drop"] = df_ptb.ecg.isin(
                df_ptb[(df_ptb.diagnosis == "Myocardial infarction") & (df_ptb.first_ecg == False)].ecg)
        ...
        if (ptb_select_only_first_mi):
            ...
            df_ptb2["drop"] = df_ptb2.ecg.isin(
                df_ptb2[(df_ptb2.diagnosis == "Myocardial infarction") & (df_ptb2.first_ecg == False)].ecg)
        ...
        if (ptb_remove_unknown_loc_mi):
            df_ptb2 = df_ptb2[
                ~((df_ptb2.diagnosis == "Myocardial infarction") & (df_ptb2.infarction_localization == ""))]
        ...
        splits_patients = get_stratified_kfolds(np.array(label_patients), n_splits=strat_folds,
        ...
        for i in range(strat_folds):
            valid_ecgs = np.array(
                df_ptb[(df_ptb.strat_fold >= 0) & (df_ptb.strat_fold == i)].ecg.apply(lambda x: x[4:]))
            train_ecgs = np.array(
                df_ptb[(df_ptb.strat_fold >= 0) & (df_ptb.strat_fold != i)].ecg.apply(lambda x: x[4:]))
...
def mat_to_np(filename_in, target_fs=100, channels=12, channel_stoi=None, fs=500, target_folder=None):
    ...
    data = resample_data(sigbufs=sigbufs, channel_stoi=channel_stoi, channel_labels=channel_labels, fs=fs,
                         target_fs=target_fs, channels=channels)
    np.save(filename_out, data.astype(np.float32))
    return sex[0], age[0, 0], channels_available, filename_out, filename_out_relative


def reformat_data_icbeb(datafiles, labelfile, target_fs=100, channels=12, channel_stoi=channel_stoi_default,
                        target_folder=None):
    # labels
    ...
    for d in tqdm(datafiles):
        sex, age, channels_available, filename_out, filename_out_relative = mat_to_np(filename_in=d,
                                                                                      target_fs=target_fs,
                                                                                      channels=channels,
                                                                                      channel_stoi=channel_stoi,
                                                                                      target_folder=target_folder)
        rows.append({"ecg": d.stem, "dataset": "ICBEB2018", "processed_data": filename_out_relative,
                     "age": (np.nan if np.isnan(age) else int(age)), "sex": sex.lower(),
...
def prepare_data_icbeb(data_folder, target_folder=None, channel_stoi=channel_stoi_default, channels=12, target_fs=50,
                       strat_folds=10, cv_random_state=42, recreate_data=False, discard_valtest=True):
    ...
    if (recreate_data):
        ...
        # reformat processed_data
        df_icbeb, lbl_itos_icbeb = reformat_data_icbeb(datafiles_icbeb, labelfile_icbeb, channel_stoi=channel_stoi,
                                                       target_fs=target_fs, channels=channels,
                                                       target_folder=target_folder)

        # TODO eventually move all the stuff below into reformat processed_data
        # remove valid processed_data
        ...
        stratified_ids = stratify(list(df_icbeb["label_multi"]), range(len(lbl_itos_icbeb)),
...
# Cell
def prepare_data_ptb_xl(data_path, min_cnt=10, target_fs=100, channels=12, channel_stoi=None,
    ...
    if recreate_data is True:
        ...
        df_ptb_xl["processed_data"] = filenames
...
# Cell
def thew_to_np(filename_in, target_fs=100, channels=12, max_length_seconds=0, channel_stoi=None, fs=180,
               target_folder=None):
    x = Holter(str(filename_in))
    metadata = {"id": x.id, "length": x.ecg_size, "record_time": str(x.record_date) + " " + str(x.start_time),
    ...
    data = resample_data(sigbufs=sigbufs, channel_stoi=channel_stoi, channel_labels=metadata["channel_labels"], fs=fs,
                         target_fs=target_fs, channels=channels)
...
def reformat_data_thew(data_path=".", max_length_seconds=0, target_fs=100, channels=12,
                       channel_stoi=channel_stoi_default, target_folder=None):
    data_path = Path(data_path)

    # prepare clinical processed_data
    ...
    for file in tqdm(list(data_path.glob('**/*.ecg'))):
        metadata, filename_out, filename_out_relative = thew_to_np(file, target_fs=target_fs, channels=channels,
                                                                   max_length_seconds=max_length_seconds,
                                                                   channel_stoi=channel_stoi, fs=180,
                                                                   target_folder=target_folder)
        res.append({"ecg_id": file.stem.replace("S-", "").replace("S", "").lstrip(),
                    "patient_id": int(file.stem.replace("S-", "").replace("S", "").lstrip()[:4]),
                    "processed_data": filename_out_relative, "length": metadata["length"],
...
def prepare_data_thew(data_folder, max_length_seconds=0, target_folder=None, channel_stoi=channel_stoi_default,
                      channels=12, target_fs=100, strat_folds=10, cv_random_state=42, recreate_data=False):
    target_root_thew = Path(".") if target_folder is None else target_folder
    if (recreate_data):
        # reformat processed_data
        df_thew, lbl_itos_thew = reformat_data_thew(data_path=data_folder, max_length_seconds=max_length_seconds,
                                                    channel_stoi=channel_stoi, target_fs=target_fs, channels=channels,
                                                    target_folder=target_folder)
        ...
        stratified_ids = stratify(patients_labels, range(6), [1. / strat_folds] * strat_folds,
...
# Cell
def prepare_data_cinc(data_path, min_cnt=10, datasets=None, target_fs=100, strat_folds=10, channels=12,
                      channel_stoi=channel_stoi_default, target_folder=None, recreate_data=True):
    ...
    if (recreate_data is True):
        dx_meta = pd.concat(
            [pd.read_csv(data_path / "dx_mapping_scored.csv"), pd.read_csv(data_path / "dx_mapping_unscored.csv")],
            sort=True)
        dx_mapping_snomed_abbrev = {a: b for [a, b] in list(
            dx_meta.apply(lambda row: [row["SNOMED CT Code"], row["Abbreviation"]], axis=1))}
        ...
        for filename in tqdm(list(data_path.glob('**/*.hea'))):
            ...
            metadata.append({"processed_data": Path(filename.stem + ".npy"), "label": labels, "sex": sex, "age": age,
                             "dataset": "cinc_" + filename.parts[-2]})
...
# Cell
def prepare_data_chapman(data_path, min_cnt=10, denoised=False, target_fs=100, strat_folds=10, channels=12,
                         channel_stoi=channel_stoi_default, target_folder=None, recreate_data=True):
    ...
    if (recreate_data is True):
        ...
        df["id"] = df.FileName
        df["processed_data"] = df.FileName.apply(lambda x: x + ".npy")
        ...
        # filter (can be reapplied at any time)
        df, lbl_itos = map_and_filter_labels(df, min_cnt=min_cnt,
        ...
        for id, row in tqdm(list(df.iterrows())):
            ...
            sigbufs = np.array(df_tmp) * 0.001  # assuming processed_data is given in muV
...
# Cell
def prepare_data_ribeiro_test(data_path, denoised=False, target_fs=100, strat_folds=10, channels=12,
                              channel_stoi=channel_stoi_default, target_folder=None, recreate_data=True):
    ...
    if (recreate_data is True):
        ...
        def reformat_predictions(df, colname_txt="label_txt", colname_num="label",
                                 lbl_itos=["1AVB", "RBBB", "LBBB", "SBRAD", "AFIB", "STACH"]):
            lbl_stoi = {s: i for i, s in enumerate(lbl_itos)}
            df[colname_txt] = df.apply(
                lambda row: ("1AVB " if row["1dAVb"] else "") + ("RBBB " if row["RBBB"] else "") + (
                    "LBBB " if row["LBBB"] else "") + ("SBRAD " if row["SB"] else "") + (
                                "AFIB " if row["AF"] else "") + ("STACH " if row["ST"] else ""), axis=1)
        ...
        df = df_gold.join(
            [df_cardiologist1, df_cardiologist2, df_cardiology_residents, df_emergency_residents, df_medical_students,
             df_dnn, df_attributes])
        df["processed_data"] = [Path("Ribeiro_test_" + str(i) + ".npy") for i in range(len(df))]
        df["dataset"] = "Ribeiro_test"
        # prepare raw processed_data
        with h5py.File(data_path / "ecg_tracings.hdf5", "r") as f:
            sigbufs = np.array(f['tracings'])
        start_idxs = [np.where(np.sum(np.abs(sigbufs[i]), axis=1) == 0.)[0] for i in
                      range(len(sigbufs))]  # discard zeros at beginning/end
        start_idxs = [len(a) // 2 for a in start_idxs]

        for id, row in tqdm(list(df.iterrows())):
            data = resample_data(sigbufs=sigbufs[id][start_idxs[id]:-start_idxs[id] or None], channel_stoi=channel_stoi,
                                 channel_labels=channel_labels, fs=fs, target_fs=target_fs, channels=channels)
            assert (target_fs <= fs)
            np.save(target_root / (row["processed_data"]), data)

        stratified_ids = stratify(list(df.apply(lambda row: row["label_txt"] + [row["sex"]], axis=1)),
                                  lbl_itos + ["male", "female"], [1. / strat_folds] * strat_folds)
        df["strat_fold"] = -1
        idxs = np.array(df.index.values)
        for i, split in enumerate(stratified_ids):
            df.loc[idxs[split], "strat_fold"] = i
...
# Cell
def prepare_data_ludb(data_path, subdivide=True, extra_segments=True, crop_bg=True, min_cnt=10, target_fs=100,
    ...
    if (recreate_data is True):
        ...
        def convert_ann(sample, symbol, length=5000, target_fs=100, fs=500, subdivide=True, extra_segments=True):
            ...
            while (i < len(symbol) - 1):
                ...
                elif (i < len(symbol) - 2 and symbol[i] == "(" and symbol[i + 1] in ["N", "t", "p"] and symbol[
                    i + 2] == ")"):
                    start = int(np.round(target_fs / fs * sample[i]))
        ...
        for filename in tqdm(list(data_path.glob('**/*.hea'))):
            ...
            assert (target_fs <= header['fs'])
            meta["processed_data"] = Path(filename.stem + ".npy")

            # read and convert annotations
            anns = [wfdb.rdann(str(filename)[:-4], "atr_" + s) for s in header["sig_name"]]
            data_ann = []
            for ann in anns:
                ann_conv, lbl_itos_ann = convert_ann(ann.sample, ann.symbol, len(sigbufs), target_fs, header["fs"],
                                                     subdivide=subdivide, extra_segments=extra_segments)
...
# Cell
def prepare_data_ribeiro_full(data_path, max_records_per_id_exam=0, min_records_per_id_exam=0,
                              overwrite_npy_if_exists=False, selected_folds=None, skip_strat_folds=False,
                              subsample=None, target_fs=100, strat_folds=100, channels=12,
                              channel_stoi=channel_stoi_default, target_folder=None, recreate_data=True):
    data_path = Path(data_path)
    ...
    if (recreate_data is True):
        ...
        if (max_records_per_id_exam > 0 or min_records_per_id_exam > 0):
            ...
            print("In total", len(df), "records from more than", len(df.id_patient.unique()),
        ...
        if (subsample is not None):
            ...
            patient_ids_selected = patient_ids[
                                   int(subsample[0] * len(patient_ids)):int(subsample[1] * len(patient_ids))]
            ...
            df_unlabeled = df_unlabeled.iloc[
                           int(subsample[0] * len(df_unlabeled)):int(subsample[1] * len(df_unlabeled))]
        ...
        if (skip_strat_folds is False):
            ...
            df_labeled["label_age"] = df_labeled.age.apply(lambda x: len(lbl_itos) + 3 if x < 20 else (
                len(lbl_itos) + 4 if x < 40 else (
                    len(lbl_itos) + 5 if x < 60 else (len(lbl_itos) + 6 if x < 80 else len(lbl_itos) + 7))))

            df_labeled["labelx"] = df_labeled.apply(
                lambda row: row["label"] + [len(lbl_itos)] + [row["label_sex"]] + [row["label_age"]], axis=1)
            ...
            stratified_ids = stratify_batched(patients_labels, range(len(lbl_itos) + 8),
                                              [1. / strat_folds] * strat_folds, samples_per_group=patients_num_ecgs,
        ...
        def process_single(i, df):
            ...
            if (overwrite_npy_if_exists or not ((target_root / (filename.stem + ".npy")).exists())):  # recreate npy
                ...
                data = resample_data(sigbufs=sigbufs, channel_stoi=channel_stoi, channel_labels=sig_names,
                                     fs=header['fs'], target_fs=target_fs, channels=channels)
        ...
        # df_result = pd.DataFrame({"id":ids,"processed_data":filenames,"record_datetime":times}).set_index("id")
        # df["id"]= range(len(df))
        # df = df.join(df_result,on="id").drop("id",axis=1)

        df["processed_data"] = filenames
...
def get_data_annot_stats(data_path, annotation=True, rhythm=True):
    ...
    if (annotation):
        unique_symbols, unique_symbols_counts = np.unique(
            [item for sublist in list(df_stats.symbol) for item in sublist], return_counts=True)
    ...
    if (rhythm and annotation):
        unique_rhythms, unique_rhythms_counts = np.unique(
            [item for sublist in list(df_stats.rhythm) for item in sublist], return_counts=True)
...
def prepare_physionet_dataset_with_annotations(df_stats, ann_stoi, ann_unk_id, dataset_name="mitdb",
                                               discard_labels=[""], rhythm=True, create_segments=True,
                                               min_len_segments=100, drop_unk=True, target_fs=100, strat_folds=10,
                                               channels=12, channel_stoi=channel_stoi_default, target_folder=None,
                                               recreate_data=True):
    """
    Two modes:
    ...
    * segments with uniform rhythm
    ...
    if (recreate_data is True):
        ...
        for sample_id, row in tqdm(df_stats.iterrows(), total=len(df_stats)):
            ...
            data = resample_data(sigbufs=sigbufs, channel_stoi=channel_stoi, channel_labels=row["channels"],
                                 fs=header['fs'], target_fs=target_fs, channels=channels)
            # print(header,processed_data[0])
            assert (header['units'][0] == 'mV')
            # processed_data = processed_data * 0.001 #fix mV???
            ...
            if (create_segments):  # create segments
                segments, segments_label = create_segment_list(len(sigbufs), ann_sample, ann_annotation, ann_stoi,
                                                               ann_unk_id, discard_labels=discard_labels)
                ...
                for i, (seg, segl) in enumerate(zip(segments, segments_label)):
                    if (seg[1] - seg[0] > 0 and (min_len_segments == 0 or seg[1] - seg[
                        0] >= min_len_segments)):  # discard segments that are too short
                        meta = {"comments": comments, "processed_data": Path(filename.stem + "_" + str(i) + ".npy"),
                                "label": segl, "label_unique": [segl], "start_index": seg[0], "end_index": seg[1],
                                "patient_id": row["patient_id"] if "patient_id" in df_stats.columns else sample_id}
            ...
            mask = create_segmentation_mask(len(sigbufs), ann_sample, ann_annotation, ann_stoi, ann_unk_id,
            ...
            meta = {"comments": comments, "processed_data": Path(filename.stem + ".npy"),
                    "label": Path(filename.stem + "_ann.npy"), "label_unique": np.unique(mask),
                    "patient_id": row["patient_id"] if "patient_id" in df_stats.columns else sample_id}
            if (create_segments):
        ...
        df_patients = (df_single if create_segments else df).groupby("patient_id")["label_unique"].apply(
            lambda x: list(x))
        patients_ids = list(df_patients.index)
        patients_labels = list(df_patients.apply(lambda x: [item for sublist in x for item in sublist]))
        patients_num_ecgs = list(df_patients.apply(len))

        stratified_ids = stratify(patients_labels, lbl_unique_single if create_segments else lbl_unique,
                                  [1. / strat_folds] * strat_folds, samples_per_group=patients_num_ecgs)
        stratified_patient_ids = [[patients_ids[i] for i in fold] for fold in stratified_ids]
        ...
        lbl_itos = [""] * int(1 + max(np.unique(list(ann_stoi.values())))) if drop_unk else [""] * int(
            1 + max(np.unique([ann_unk_id] + list(ann_stoi.values()))))
...
def prepare_mitdb(data_path, ann_stoi=ann_stoi_af, ann_unk_id=ann_unk_id_af, create_segments=True, drop_unk=False,
                  target_fs=128, strat_folds=5, channels=2, channel_stoi=channel_stoi_mitdb, target_folder=None,
                  recreate_data=True):
    ...
    print("\n\nProcessing records...")
    return prepare_physionet_dataset_with_annotations(df_stats, ann_stoi, ann_unk_id, dataset_name="mitdb",
                                                      discard_labels=[""], rhythm=True, create_segments=create_segments,
                                                      drop_unk=drop_unk, target_fs=target_fs, strat_folds=strat_folds,
                                                      channels=channels, channel_stoi=channel_stoi,
                                                      target_folder=target_folder, recreate_data=recreate_data)
...
def prepare_afdb(data_path, ann_stoi=ann_stoi_af, ann_unk_id=ann_unk_id_af, create_segments=False, drop_unk=False,
                 target_fs=128, strat_folds=5, channels=2, channel_stoi=channel_stoi_afdb, target_folder=None,
                 recreate_data=True):
    ...
    print("\n\nProcessing records...")
    return prepare_physionet_dataset_with_annotations(df_stats, ann_stoi, ann_unk_id, dataset_name="afdb",
                                                      discard_labels=[""], rhythm=True, create_segments=create_segments,
                                                      drop_unk=drop_unk, target_fs=target_fs, strat_folds=strat_folds,
                                                      channels=channels, channel_stoi=channel_stoi,
                                                      target_folder=target_folder, recreate_data=recreate_data)
...
def prepare_ltafdb(data_path, ann_stoi=ann_stoi_af, ann_unk_id=ann_unk_id_af, create_segments=False, drop_unk=False,
                   target_fs=128, strat_folds=5, channels=2, channel_stoi=channel_stoi_afdb, target_folder=None,
                   recreate_data=True):
    ...
    print("\n\nProcessing records...")
    return prepare_physionet_dataset_with_annotations(df_stats, ann_stoi, ann_unk_id, dataset_name="ltafdb",
                                                      discard_labels=[""], rhythm=True, create_segments=create_segments,
                                                      drop_unk=drop_unk, target_fs=target_fs, strat_folds=strat_folds,
                                                      channels=channels, channel_stoi=channel_stoi,
                                                      target_folder=target_folder, recreate_data=recreate_data)
...
def prepare_vfdb(data_path, ann_stoi=ann_stoi_af, ann_unk_id=ann_unk_id_af, create_segments=False, drop_unk=False,
                 target_fs=128, strat_folds=5, channels=2, channel_stoi=channel_stoi_afdb, target_folder=None,
                 recreate_data=True):
    ...
    print("\n\nProcessing records...")
    return prepare_physionet_dataset_with_annotations(df_stats, ann_stoi, ann_unk_id, dataset_name="vfdb",
                                                      discard_labels=[""], rhythm=True, create_segments=create_segments,
                                                      drop_unk=drop_unk, target_fs=target_fs, strat_folds=strat_folds,
                                                      channels=channels, channel_stoi=channel_stoi,
                                                      target_folder=target_folder, recreate_data=recreate_data)
...
def prepare_cpsc21(data_path, ann_stoi=ann_stoi_af, ann_unk_id=ann_unk_id_af, create_segments=False, drop_unk=False,
                   target_fs=128, strat_folds=5, channels=2, channel_stoi=channel_stoi_cpsc21, target_folder=None,
                   recreate_data=True):
    ...
    print("\n\nProcessing records...")
    return prepare_physionet_dataset_with_annotations(df_stats, ann_stoi, ann_unk_id, dataset_name="cpsc21",
                                                      discard_labels=[""], rhythm=True, create_segments=create_segments,
                                                      drop_unk=drop_unk, target_fs=target_fs, strat_folds=strat_folds,
                                                      channels=channels, channel_stoi=channel_stoi,
                                                      target_folder=target_folder, recreate_data=recreate_data)
