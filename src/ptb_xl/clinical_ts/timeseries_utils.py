...
tsdata_static = namedtuple("tsdata_static", ("processed_data", "label", "static"))
tsdata = namedtuple("tsdata", ("processed_data", "label"))
...
def apply_butter_filter(data, filter, forwardbackward=True):
    """pass filter from call of butter_filter to processed_data (assuming time axis at dimension 0)"""
    if forwardbackward:
...
def dataset_add_chunk_col(df, col="processed_data"):
    '''add a chunk column to the dataset df'''
    df["chunk"] = df.groupby(col).cumcount()


def dataset_add_length_col(df, col="processed_data", data_folder=None):
...
def dataset_add_labels_col(df, col="label", data_folder=None):
    ...
    df[col + "_labels"] = df[col].apply(
        lambda x: list(np.unique(np.load(x if data_folder is None else data_folder / x, allow_pickle=True))))


def dataset_add_mean_col(df, col="processed_data", axis=(0), data_folder=None):
...
def dataset_add_median_col(df, col="processed_data", axis=(0), data_folder=None):
...
def dataset_add_std_col(df, col="processed_data", axis=(0), data_folder=None):
    '''adds a column with mean'''
    df[col + "_std"] = df[col].apply(
        lambda x: np.std(np.load(x if data_folder is None else data_folder / x, allow_pickle=True), axis=axis))


def dataset_add_iqr_col(df, col="processed_data", axis=(0), data_folder=None):
    '''adds a column with mean'''
    df[col + "_iqr"] = df[col].apply(
        lambda x: iqr(np.load(x if data_folder is None else data_folder / x, allow_pickle=True), axis=axis))


def dataset_get_stats(df, col="processed_data", simple=True):
...
def reformat_as_memmap(df, target_filename, data_folder=None, annotation=False, max_len=0, delete_npys=True,
                       col_data="processed_data", col_label="label", batch_length=0):
    ...
    # replace processed_data(filename) by integer
    df_mapped = df.copy()
...
class ConcatDatasetTimeseriesDatasetCrops(torch.utils.data.ConcatDataset):
    ...
    def __init__(self, datasets):
        ...
        for dataset_idx, ds in enumerate(self.datasets):
            ...
            remap_dict = {x: j + (self.cumulative_sizes[dataset_idx - 1] if dataset_idx > 0 else 0) for j, x in
                          enumerate(np.unique(idmap))}
...
class TimeseriesDatasetCrops(torch.utils.data.Dataset):
    ...
    def __init__(self, df, output_size, chunk_length, min_chunk_length, memmap_filename=None, npy_data=None,
                 random_crop=True, data_folder=None, num_classes=2, copies=0, col_lbl="label", cols_static=None,
                 stride=None, start_idx=0, annotation=False, transforms=None, sample_items_per_record=1):
        """
        accepts three kinds of input:
        1) filenames pointing to aligned numpy arrays [timesteps,channels,...] for processed_data and either integer labels or filename pointing to numpy arrays[timesteps,...] e.g. for annotations
        2) memmap_filename to memmap file (same argument that was passed to reformat_as_memmap) for processed_data [concatenated,...] and labels- processed_data column in df corresponds to index in this memmap
        3) npy_data [samples,ts,...] (either path or np.array directly- also supporting variable length input) - processed_data column in df corresponds to sampleid
        ...
        # keys (in column processed_data) have to be unique
        assert (len(df["processed_data"].unique()) == len(df))

        self.timeseries_df_data = np.array(df["processed_data"])
        if (self.timeseries_df_data.dtype not in [np.int16, np.int32, np.int64]):
            ...
            self.timeseries_df_data = np.array(df["processed_data"].astype(str)).astype(np.string_)
        ...
        for df_idx, (id, row) in enumerate(df.iterrows()):
            if (self.mode == "files"):
                ...
                data_length = self.memmap_length[row["processed_data"]]
            else:  # npy
                data_length = len(self.npy_data[row["processed_data"]])
    ...
    def _getitem(self, idx, start_idx_rel):
        # low-level function that actually fetches the processed_data
        ...
        # load the actual processed_data
        if (self.mode == "files"):  # from separate files
            ...
            data = np.load(data_filename, allow_pickle=True)[
                   start_idx_crop:end_idx_crop]  # processed_data type has to be adjusted when saving to npy
            ...
            # wi = torch.utils.processed_data.get_worker_info()
        ...
        # consistency check: make sure that processed_data and annotation lengths match
    ...
    def __call__(self, sample):
        if self.scale == 0:
            ...
            data = data + np.reshape(np.array([random.gauss(0, self.scale) for _ in range(np.prod(data.shape))]),
                                     data.shape)  # np.random.real(scale=self.scale,size=processed_data.shape).astype(np.float32)
            return data, label, static
...
class NormalizeBatch(object):
    ...
    def __call__(self, sample):
        ...
        data = datax if self.input else labelx
        # assuming channel last
        # batch_mean = np.mean(processed_data,axis=tuple(range(0,len(processed_data)-1)))
        # batch_std = np.std(processed_data,axis=tuple(range(0,len(processed_data)-1)))+1e-8
        batch_mean = np.mean(data, axis=self.axis if self.axis is not None else tuple(range(0, len(data.shape) - 1)))
        batch_std = np.std(data,
                           axis=self.axis if self.axis is not None else tuple(range(0, len(data.shape) - 1))) + 1e-8
...
class Transform(object):
    """Transforms processed_data using a given function i.e. data_new = func(processed_data) for input is True else label_new = func(label)
    """
...
class TupleTransform(object):
    """Transforms processed_data using a given function (operating on both processed_data and label and return a tuple) i.e. data_new, label_new = func(data_old, label_old)
    """
